...
export const mapTrendingVideosAndSimilar = async (
  ...
  const videosUrl = `https://api.themoviedb.org/3/${mediaType}/${el.id}/videos?api_key=${process.env.NEXT_PUBLIC_MOVIE_DB_API_KEY}&language=en-US`;
  ...
  const similarUrl = `https://api.themoviedb.org/3/${mediaType}/${el.id}/similar?api_key=${process.env.NEXT_PUBLIC_MOVIE_DB_API_KEY}&language=en-US&page=1`;
